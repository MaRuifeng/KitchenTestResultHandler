...
<xsl:param name="TITLE">[<xsl:value-of select="testsuites/@testCat"/>] Chef Recipe Kitchen Test Report (Continuous Compliance &amp; Self Service Delivery)</xsl:param>
...
<xsl:template match="testsuites">
    ...
    <xsl:template name="summary">
        ...
        <xsl:variable name="totalExeDuration" select="sum(testsuite/@time)"/>
        <xsl:variable name="totalPrepDuration" select="sum(testsuite/@prepTime)"/>
        <xsl:variable name="totalTeardownDuration" select="sum(testsuite/@teardownTime)"/>
        <xsl:variable name="totalDuration" select="$totalExeDuration + $totalPrepDuration + $totalTeardownDuration"/>
        <xsl:variable name="successRate" select="($testCount - $failureCount - $errorCount) div $testCount"/>
        <xsl:variable name="logsLink" select="../testsuites/@logs"/>
        <xsl:variable name="testEdptProvider" select="../testsuites/@testEdptProvider"/>
        ...
        <tr valign="top">
            ...
            <td>
                <xsl:call-template name="display-time">
                    <xsl:with-param name="value" select="$totalPrepDuration"/>
                </xsl:call-template>
            </td>
            <td>
                <xsl:call-template name="display-time">
                    <xsl:with-param name="value" select="$totalExeDuration"/>
                </xsl:call-template>
            </td>
            <td>
                <xsl:call-template name="display-time">
                    <xsl:with-param name="value" select="$totalTeardownDuration"/>
                </xsl:call-template>
            </td>
            <td>
                <xsl:call-template name="display-time">
                    <xsl:with-param name="value" select="$totalDuration"/>
                </xsl:call-template>
            </td>
            <td><a href="{$logsLink}" target="_blank">Logs</a></td>
            <td><xsl:value-of select="$testEdptProvider"/></td>
        </tr>
        </table>
    ...
    <xsl:template name="packagelist">
        <h2>Test Packages (Policy Cookbooks) </h2>
        <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
            ...
            <xsl:for-each select="./testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
                ...
                <xsl:variable name="prepTimeCount" select="sum($testsuites-in-package/@prepTime)"/>
                <xsl:variable name="timeCount" select="sum($testsuites-in-package/@time)"/>
                <xsl:variable name="teardownTimeCount" select="sum($testsuites-in-package/@teardownTime)"/>
                ...
                <tr valign="top">
                    ...
                    <td>
	                    <xsl:call-template name="display-time">
	                        <xsl:with-param name="value" select="$prepTimeCount"/>
	                    </xsl:call-template>
                    </td>
                    <td>
	                    <xsl:call-template name="display-time">
                    ...
                    <td>
	                    <xsl:call-template name="display-time">
	                        <xsl:with-param name="value" select="$teardownTimeCount"/>
	                    </xsl:call-template>
                    </td>
                    ...
                    <td><a href="{$logLink}" target="_blank">test.log</a></td>
        ...
        <table border="0" width="95%">
          <tr>
          <td style="text-align: justify;">
              Note: policies are grouped by platforms and each group is taken as a test package with statistics computed to alert its owner.
          </td>
          </tr>
        </table>
    </xsl:template>
...
<xsl:template name="testsuite.test.header">
    <tr valign="top">
        ...
        <th>Pass Rate</th>
        <th nowrap="nowrap">Prep Time</th>
        <th nowrap="nowrap">Exe Time</th>
        <th nowrap="nowrap">Tear Time</th>
...
<xsl:template name="testcase.test.header">
    <tr valign="top">
        ...
        <th>Endpoint OS</th>
        <th>Endpoint IP</th>
        <th nowrap="nowrap">Preparation Time</th>
        <th nowrap="nowrap">Execution Time</th>
        <th nowrap="nowrap">Teardown Time</th>
    </tr>
</xsl:template>
...
<xsl:template match="testcase" mode="print.test">
    <tr valign="top">
        ...
        <xsl:variable name="logLink" select="@log"/>
        <td><a href="{$logLink}" target="_blank">test.log</a></td>
        <xsl:variable name="testEdptOS" select="@testEdptOS"/>
        <td><xsl:value-of select="$testEdptOS"/></td>
        <xsl:variable name="testEdptIP" select="@testEdptIP"/>
        <td><xsl:value-of select="$testEdptIP"/></td>
        <td>
            <xsl:call-template name="display-time">
                <xsl:with-param name="value" select="@prepTime"/>
            </xsl:call-template>
        </td>
        <td>
            <xsl:call-template name="display-time">
        ...
        <td>
            <xsl:call-template name="display-time">
                <xsl:with-param name="value" select="@teardownTime"/>
            </xsl:call-template>
        </td>
    </tr>
</xsl:template>
